...
class MongoFs
{
	...
	protected $_collectionFolders = 'appmanager.files';
	protected $_collectionFs = 'appmanager';
	...
	{
        $filename = $this->strip_dslashes($filename);
	...
	public function import($filename, $realfile, $options = null)
	{
		...
		$meta = array(
				...
				'parent' => $parent,
				'type' => 'file',
				//'mimetype' => GFileHelper::getMimeTypeByExtension($realfile), @todo!
				'meta' => $options
		);
		if(isset($id))
			$meta['_id'] = $id;
    ...
    private function strip_dslashes($str){
        $str = str_replace("//","/",$str);
        return $str;
	 ...
	 * @return Returns TRUE on success or FALSE on failure.
	 */
	public function copy($source, $dest,$tmp){
		$this->export($source,$tmp);
		$this->import($dest,$tmp);
			
	}
	


    public function export($source,$dest){
        $source = $this->strip_dslashes($source);
        $f = fopen($dest,"w");
        if(!$f){ return false;}
        $fsource = $this->file_get_contents($source);
        fwrite($f,$fsource);
        fclose($f);
        return true;
